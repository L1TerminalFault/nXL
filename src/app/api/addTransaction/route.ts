...
export async function POST(req: Request) {
  ...
  try {
    ...
    let cleanText = raw.replace(/[\x00-\x1F\x7F-\x9F]/g, "");
    ...
    // Otherwise expect standard SMS payload format
    const { transaction } = parsedObj;

    if (!transaction || typeof transaction !== "string") {
      return Response.json({ status: "error", message: "Invalid transaction format" }, { status: 400 });
    }
